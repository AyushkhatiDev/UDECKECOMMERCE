...
// const dealProducts = [
//     { id: 901, name: "Luxury Winter Coat", oldPrice: 199.99, price: 129.99, discount: 35, imageUrl: "https://images.unsplash.com/photo-1544969265401-82b9e0d0a1e2?w=400&h=300&fit=crop&crop=center", imageColor: "6B7280", category: "Fashion", brand: "StyleCraft", rating: 4 },
...
//             ? `<img src="${item.imageUrl}" alt="${item.name}" class="w-16 h-16 md:w-20 md:h-20 object-cover rounded-lg" onerror="this.onerror=null; this.outerHTML='<div class=&quot;w-16 h-16 md:w-20 md:h-20 rounded-lg&quot; style=&quot;background-color:${colorHex}&quot;></div>'" />`
//             : `<div class="w-16 h-16 md:w-20 md:h-20 rounded-lg" style="background-color:${colorHex}"></div>`;
//         const subtotal = +(item.price * item.quantity).toFixed(2);
//         return `
//         <div class="bg-white p-3 md:p-4 rounded-xl shadow-lg border border-card-border flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
//             <div class="flex items-start space-x-3 md:space-x-4">
//                 ${imageMarkup}
//                 <div class="flex-1 min-w-0">
//                     <h4 class="font-semibold text-primary text-sm md:text-base line-clamp-2">${item.name}</h4>
//                     <p class="text-xs md:text-sm text-gray-500">Per-unit: ${formatCurrency(item.price)}</p>
//                 </div>
//             </div>
//             <div class="flex items-center justify-between space-x-2 md:space-x-4">
//                 <div class="flex items-center space-x-1 md:space-x-2">
//                     <button class="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gray-200 text-primary font-bold flex items-center justify-center text-sm" onclick="decCartItem(${item.id})">-</button>
//                     <input id="cart-qty-${item.id}" type="number" min="1" max="500" value="${item.quantity}" class="w-14 md:w-16 p-1 md:p-2 border rounded-md text-center text-xs md:text-sm" oninput="setCartItemQuantity(${item.id}, this.value)" />
//                     <button class="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gray-200 text-primary font-bold flex items-center justify-center text-sm" onclick="incCartItem(${item.id})">+</button>
//                 </div>
//                 <div class="text-right min-w-0 flex-shrink-0">
//                     <p class="font-semibold text-primary text-sm md:text-base">${formatCurrency(subtotal)}</p>
//                     <button class="text-xs md:text-sm text-secondary-accent hover:text-red-700 mt-1 block" onclick="removeFromCart(${item.id})">Remove</button>
//                 </div>
//             </div>
//         </div>`;
//     }).join('');
...
//     for (let i = 1; i <= 5; i++) {
//         if (i <= rating) {
//             stars += `<i data-lucide="star" class="w-3 h-3 md:w-4 md:h-4 fill-yellow-400 text-yellow-400"></i>`;
//         } else {
//             stars += `<i data-lucide="star" class="w-3 h-3 md:w-4 md:h-4 text-gray-300"></i>`;
//         }
//     }
//     return stars;
// }
...
//                 <i data-lucide="heart" class="w-12 h-12 md:w-16 md:h-16 text-gray-300 mx-auto mb-4"></i>
//                 <h3 class="text-lg md:text-xl font-semibold text-gray-600 mb-2">Your wishlist is empty</h3>
//                 <p class="text-gray-500 mb-6 text-sm">Add items you love to your wishlist</p>
//                 <a href="#" onclick="navigateTo('home')" class="bg-accent text-white px-4 md:px-6 py-2 rounded-lg hover:bg-accent-dark transition duration-150 text-sm">
//                     Continue Shopping
//                 </a>
//             </div>
//         `;
...
//             return `
//                 <div class="bg-white rounded-lg shadow-sm border p-3 md:p-4 flex flex-col md:flex-row md:items-center space-y-3 md:space-y-0 md:space-x-4">
//                     <div class="w-20 h-20 md:w-16 md:h-16 bg-gray-100 rounded-lg flex items-center justify-center overflow-hidden flex-shrink-0">
//                         ${product.imageUrl ? 
...
//                     </div>
//                     <div class="flex-1 min-w-0">
//                         <h4 class="font-semibold text-gray-800 text-sm md:text-base line-clamp-2">${product.name}</h4>
//                         <p class="text-accent font-bold text-sm">${formatCurrency(product.price)}</p>
//                     </div>
//                     <div class="flex items-center space-x-2 md:space-x-3">
//                         <button onclick="addToCart(${product.id})" class="bg-accent text-white px-3 md:px-4 py-2 rounded-lg hover:bg-accent-dark transition duration-150 text-xs md:text-sm flex-1 md:flex-none">
//                             Add to Cart
//                         </button>
//                         <button onclick="toggleWishlist(${product.id})" class="text-red-500 hover:text-red-700 p-2 md:p-1.5 flex items-center justify-center">
...
//         container.innerHTML = `
//             <div class="space-y-4">
//                 <h3 class="text-lg md:text-xl font-semibold text-gray-800 mb-4">My Wishlist (${wishlistItems.length} items)</h3>
//                 ${wishlistHTML}
//             </div>
//         `;
//     }
...
//     container.innerHTML = `
//         <div class="space-y-6">
//             <h3 class="text-lg md:text-xl font-semibold text-gray-800">Account Dashboard</h3>
//             <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
//                 <div class="bg-white rounded-lg shadow-sm border p-4 md:p-6">
//                     <div class="flex items-center space-x-3 mb-4">
//                         <i data-lucide="shopping-bag" class="w-6 h-6 md:w-8 md:h-8 text-accent"></i>
//                         <div>
//                             <h4 class="font-semibold text-gray-800 text-sm md:text-base">Total Orders</h4>
//                             <p class="text-xl md:text-2xl font-bold text-accent">0</p>
//                         </div>
//                     </div>
//                     <p class="text-gray-600 text-xs md:text-sm">No orders yet</p>
//                 </div>
//                 <div class="bg-white rounded-lg shadow-sm border p-4 md:p-6">
//                     <div class="flex items-center space-x-3 mb-4">
//                         <i data-lucide="heart" class="w-6 h-6 md:w-8 md:h-8 text-red-500"></i>
//                         <div>
//                             <h4 class="font-semibold text-gray-800 text-sm md:text-base">Wishlist Items</h4>
//                             <p class="text-xl md:text-2xl font-bold text-red-500">${wishlistItems.length}</p>
//                         </div>
//                     </div>
//                     <p class="text-gray-600 text-xs md:text-sm">${wishlistItems.length === 0 ? 'No items in wishlist' : 'Items saved for later'}</p>
//                 </div>
//                 <div class="bg-white rounded-lg shadow-sm border p-4 md:p-6">
//                     <div class="flex items-center space-x-3 mb-4">
//                         <i data-lucide="shopping-cart" class="w-6 h-6 md:w-8 md:h-8 text-green-500"></i>
//                         <div>
//                             <h4 class="font-semibold text-gray-800 text-sm md:text-base">Cart Items</h4>
//                             <p class="text-xl md:text-2xl font-bold text-green-500">${cartItems.length}</p>
//                         </div>
//                     </div>
//                     <p class="text-gray-600 text-xs md:text-sm">${cartItems.length === 0 ? 'Cart is empty' : 'Items ready to checkout'}</p>
//                 </div>
//             </div>
//         </div>
//     `;
...
//     container.innerHTML = `
//         <div class="space-y-6">
//             <h3 class="text-lg md:text-xl font-semibold text-gray-800">Order History</h3>
//             <div class="text-center py-8 md:py-12">
//                 <i data-lucide="package" class="w-12 h-12 md:w-16 md:h-16 text-gray-300 mx-auto mb-4"></i>
//                 <h4 class="text-base md:text-lg font-semibold text-gray-600 mb-2">No orders yet</h4>
//                 <p class="text-gray-500 mb-6 text-sm">When you place orders, they'll appear here</p>
//                 <a href="#" onclick="navigateTo('home')" class="bg-accent text-white px-4 md:px-6 py-2 rounded-lg hover:bg-accent-dark transition duration-150 text-sm">
//                     Start Shopping
//                 </a>
//             </div>
...
//     container.innerHTML = `
//         <div class="space-y-6">
//             <h3 class="text-lg md:text-xl font-semibold text-gray-800">Profile Information</h3>
//             <div class="bg-white rounded-lg shadow-sm border p-4 md:p-6">
//                 <form class="space-y-4">
//                     <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
//                         <div>
//                             <label class="block text-sm font-medium text-gray-700 mb-2">First Name</label>
//                             <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent text-sm" placeholder="Enter first name">
//                         </div>
//                         <div>
//                             <label class="block text-sm font-medium text-gray-700 mb-2">Last Name</label>
//                             <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent text-sm" placeholder="Enter last name">
//                         </div>
//                     </div>
//                     <div>
//                         <label class="block text-sm font-medium text-gray-700 mb-2">Email</label>
//                         <input type="email" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent text-sm" placeholder="Enter email">
//                     </div>
//                     <div>
//                         <label class="block text-sm font-medium text-gray-700 mb-2">Phone</label>
//                         <input type="tel" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-accent text-sm" placeholder="Enter phone number">
//                     </div>
//                     <button type="submit" class="bg-accent text-white px-6 py-2 rounded-lg hover:bg-accent-dark transition duration-150 w-full md:w-auto text-sm">
//                         Update Profile
//                     </button>
//                 </form>
//             </div>
...
//     container.innerHTML = `
//         <div class="space-y-6">
//             <h3 class="text-lg md:text-xl font-semibold text-gray-800">Shipping Addresses</h3>
//             <div class="text-center py-8 md:py-12">
//                 <i data-lucide="map-pin" class="w-12 h-12 md:w-16 md:h-16 text-gray-300 mx-auto mb-4"></i>
//                 <h4 class="text-base md:text-lg font-semibold text-gray-600 mb-2">No addresses saved</h4>
//                 <p class="text-gray-500 mb-6 text-sm">Add a shipping address for faster checkout</p>
//                 <button class="bg-accent text-white px-6 py-2 rounded-lg hover:bg-accent-dark transition duration-150 w-full md:w-auto text-sm">
//                     Add Address
//                 </button>
//             </div>
//         </div>
...
//     container.innerHTML = `
//         <div class="space-y-6">
//             <h3 class="text-lg md:text-xl font-semibold text-gray-800">Payment Methods</h3>
//             <div class="text-center py-8 md:py-12">
//                 <i data-lucide="credit-card" class="w-12 h-12 md:w-16 md:h-16 text-gray-300 mx-auto mb-4"></i>
//                 <h4 class="text-base md:text-lg font-semibold text-gray-600 mb-2">No payment methods saved</h4>
//                 <p class="text-gray-500 mb-6 text-sm">Add a payment method for faster checkout</p>
//                 <button class="bg-accent text-white px-6 py-2 rounded-lg hover:bg-accent-dark transition duration-150 w-full md:w-auto text-sm">
//                     Add Payment Method
//                 </button>
//             </div>
...
//     header.innerHTML = `
//         <h2 class="text-2xl md:text-3xl font-bold text-primary">${categoryName} (${products.length} items)</h2>
//         <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center text-sm text-gray-600 space-y-2 sm:space-y-0 sm:space-x-4 mt-2">
//             <span>Sort By: </span>
//             <select class="p-2 border border-card-border rounded-md w-full sm:w-auto">
...
//     banner.innerHTML = `
//         <div class="p-4 md:p-8 bg-white rounded-xl shadow-lg border-2 border-accent">
//             <p class="text-lg md:text-2xl font-bold text-primary">Explore our exclusive selection of <span class="text-accent">${categoryName}</span>!</p>
//             <p class="text-gray-600 mt-2 text-sm">Hand-picked items with our quality guarantee.</p>
//         </div>
//     `;
...
//             ? `<img src="${p.imageUrl}" alt="${p.name}" class="w-full h-40 md:h-48 object-cover rounded-lg" onerror="this.onerror=null; this.outerHTML='<div class=\\'w-full h-40 md:h-48 rounded-lg\\' style=\\'background-color:${colorHex}\\'></div>'" />`
//             : `<div class="w-full h-40 md:h-48 rounded-lg" style="background-color:${colorHex}"></div>`;
        
//         return `
//             <div class="bg-white p-3 md:p-4 rounded-xl shadow-lg border border-card-border reveal w-full">
//                 <div class="relative">
//                     ${imageElement}
...
//                 </div>
//                 <div class="mt-4">
//                     <h4 class="font-semibold text-primary text-sm md:text-base line-clamp-2">${p.name}</h4>
//                     <p class="text-xs md:text-sm text-gray-500">${p.brand || 'Brand'}</p>
//                     <div class="flex items-center mt-2">
//                         ${generateStarRating(p.rating || 4)}
//                         <span class="text-xs md:text-sm text-gray-500 ml-2">(${computeSalesCount(p)})</span>
//                     </div>
//                     <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-4 space-y-3 sm:space-y-0">
//                         <div class="w-full sm:w-auto">
//                             <span class="text-base md:text-lg font-bold text-primary">${formatCurrency(p.price)}</span>
//                             <span class="text-xs md:text-sm text-gray-500">/${p.unit || 'ea'}</span>
//                         </div>
//                         <div class="flex items-center space-x-2 w-full sm:w-auto justify-between sm:justify-normal">
//                             <button class="w-8 h-8 rounded-full bg-gray-200 text-primary font-bold flex items-center justify-center flex-shrink-0" onclick="decCategoryQty(${p.id})">-</button>
//                             <input id="category-qty-${p.id}" type="number" min="1" max="500" value="1" class="w-16 p-1 md:p-2 border rounded-md text-center text-xs" oninput="onCategoryQtyChange(${p.id}, this.value)" />
//                             <button class="w-8 h-8 rounded-full bg-gray-200 text-primary font-bold flex items-center justify-center flex-shrink-0" onclick="incCategoryQty(${p.id})">+</button>
//                             <button class="bg-accent text-white px-3 md:px-4 py-2 rounded-lg hover:bg-secondary-accent transition-colors text-xs md:text-sm flex items-center justify-center flex-1 sm:flex-none ml-2" onclick="addCategoryToCart(${p.id})">
//                                 <i data-lucide="shopping-cart" class="w-3 h-3 md:w-4 md:h-4 mr-1"></i>
//                                 <span>Add</span>
//                             </button>
//                         </div>
//                     </div>
...
//     const input = document.getElementById(`category-qty-${productId}`);
...
//             ? `<img src="${p.imageUrl}" alt="${p.name}" class="w-full aspect-square object-cover bg-white" onerror="this.onerror=null; this.outerHTML='<div class=&quot;w-full aspect-square bg-white&quot;></div>'" />`
//             : `<div class="w-full aspect-square bg-white"></div>`;
//         return `
//             <div id="product-card-${p.id}" class="w-full sm:w-64 sm:min-w-[16rem] bg-white rounded-xl shadow border border-card-border overflow-hidden product-card-hover reveal flex flex-col justify-between h-[380px] md:h-[420px] flex-none snap-start" style="transition-delay: ${i * 80}ms">
//                 <div class="relative">
...
//                         <h4 class="text-sm md:text-base font-semibold text-primary truncate-2-lines">${p.name}</h4>
//                     </button>
//                     <div class="flex items-center justify-between mt-2">
//                         <div>
//                             <span id="arr-price-${p.id}" class="text-primary font-bold text-sm md:text-base">${formatCurrency(unitPrice)}</span>
//                             ${p.unit ? `<span class="text-xs md:text-sm text-gray-500"> / ${p.unit}</span>` : ''}
//                         </div>
//                         <div class="flex">${generateStarRating(p.rating || 0)}</div>
//                     </div>
//                 <div class="mt-auto flex items-center justify-between gap-2 md:gap-3 action-bar-fixed-height h-10">
//                     <div class="flex items-center border border-card-border rounded-full overflow-hidden w-full md:w-36 h-full">
//                         <button onclick="decArrivalsQty(${p.id}); return false;" class="px-2 md:px-3 h-full text-primary hover:text-secondary-accent flex items-center justify-center text-sm">-</button>
//                         <input id="arr-qty-input-${p.id}" type="number" min="1" value="1" class="flex-1 md:w-16 text-center outline-none h-full text-xs md:text-sm" oninput="onArrivalsQtyChange(${p.id}, this.value)" />
//                         <button onclick="incArrivalsQty(${p.id}); return false;" class="px-2 md:px-3 h-full text-primary hover:text-secondary-accent flex items-center justify-center text-sm">+</button>
//                     </div>
//                     <button onclick="addBulkToCart(${p.id}); return false;" class="flex-1 text-xs md:text-sm text-white bg-accent h-full px-2 md:px-4 rounded-lg hover:bg-primary transition">Add to Cart</button>
//                 </div>
//                 </div>
//             </div>`;
//     }).join('');
...
//                     <div class="p-3 md:p-4 bg-secondary-accent text-white">
//                         <p class="text-xs md:text-sm font-semibold tracking-wide">Bulk Offer</p>
//                         <h4 class="text-base md:text-lg font-extrabold">${tierText}</h4>
//                     </div>
//                     <div class="p-3 md:p-4">
//                         <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0">
//                             <div class="min-w-0">
//                                 <p class="text-primary font-semibold text-sm md:text-base line-clamp-2">${p.name}</p>
//                                 <p class="text-gray-500 text-xs">${p.category} · ${p.brand}</p>
//                             </div>
//                             <span class="text-accent font-bold text-sm md:text-base">${formatCurrency(p.price)}</span>
//                         </div>
...
//         const colorHex = (p.imageColor ? `#${p.imageColor}` : '#e5e7eb');
//         const imageMarkup = p.imageUrl
//             ? `<div class="w-full h-32 md:h-48 overflow-hidden bg-white flex items-center justify-center">
...
//                </div>`
//             : `<div class="w-full h-32 md:h-48 bg-gray-200"></div>`;
//         const initialQty = 1;
//         const unitTotal = calculateBulkPrice(p, initialQty);
//         const unitPrice = +(unitTotal / initialQty).toFixed(2);
//         return `
//         <div id="product-card-${p.id}" class="bg-white rounded-xl shadow-lg overflow-hidden product-card-hover border border-card-border reveal flex flex-col h-[380px] md:h-[420px] w-full" style="transition-delay: ${i * 80}ms">
//             <div class="relative">
...
//                     <h4 class="text-base md:text-lg font-semibold text-primary truncate-2-lines">${p.name}</h4>
//                     <div class="flex items-baseline space-x-2 mt-2">
//                         <span id="rec-price-${p.id}" class="text-primary font-bold text-sm md:text-base">${formatCurrency(unitPrice)}</span>
//                         ${p.unit ? `<span class="text-gray-500 text-xs md:text-sm">/ ${p.unit}</span>` : ''}
//                     </div>
//                     <div class="flex items-center mt-2">
//                         ${generateStarRating(p.rating || 0)}
//                     </div>
//                 </div>
//                 <div class="flex items-center justify-between gap-2 md:gap-3 h-10 mt-3">
//                     <div class="flex items-center border border-card-border rounded-full overflow-hidden w-full md:w-36 h-full">
//                         <button onclick="decRecommendedQty(${p.id}); return false;" class="px-2 md:px-3 h-full text-primary hover:text-secondary-accent flex items-center justify-center text-sm">-</button>
//                         <input id="rec-qty-input-${p.id}" type="number" min="1" value="1" class="flex-1 md:w-16 text-center outline-none h-full text-xs md:text-sm" oninput="onRecommendedQtyChange(${p.id}, this.value)" />
//                         <button onclick="incRecommendedQty(${p.id}); return false;" class="px-2 md:px-3 h-full text-primary hover:text-secondary-accent flex items-center justify-center text-sm">+</button>
//                     </div>
//                     <button onclick="addBulkToCart(${p.id}); return false;" class="flex-1 px-2 md:px-4 h-full bg-accent text-white rounded-lg hover:bg-primary transition flex items-center justify-center text-xs md:text-sm">Add to Cart</button>
//                 </div>
//             </div>
//         </div>`;
//     }).join('');
...
//         ? `<img src="${elect.imageUrl}" alt="Electronics" class="w-full h-40 md:h-[280px] object-cover" onerror="this.onerror=null; this.outerHTML='<div class=&quot;w-full h-40 md:h-[280px]&quot; style=&quot;background-color:${electColor}&quot;></div>'" />`
//         : `<div class="w-full h-40 md:h-[280px]" style="background-color:${electColor}"></div>`;
...
//         ? `<img src="${fash.imageUrl}" alt="Fashion" class="w-full h-40 md:h-[280px] object-cover" onerror="this.onerror=null; this.outerHTML='<div class=&quot;w-full h-40 md:h-[280px]&quot; style=&quot;background-color:${fashionColor}&quot;></div>'" />`
//         : `<div class="w-full h-40 md:h-[280px]" style="background-color:${fashionColor}"></div>`;
//     categoryHighlights.innerHTML = `
//         <a href="#category-electronics" onclick="navigateTo('category','electronics'); return false;" class="block w-full md:w-1/2">
...
//                     <h3 class="text-xl md:text-2xl font-extrabold text-white">Electronics</h3>
//                     <p class="text-white/90 text-sm">TVs, Audio, Accessories</p>
//                 </div>
//             </div>
//         </a>
//         <a href="#category-fashion" onclick="navigateTo('category','fashion'); return false;" class="block w-full md:w-1/2">
//             <div class="relative rounded-xl overflow-hidden shadow-lg border border-card-border hover:shadow-xl transition">
//                 ${fashionMarkup}
//                 <div class="absolute inset-0 bg-black/20"></div>
//                 <div class="absolute bottom-3 md:bottom-4 left-3 md:left-4">
//                     <h3 class="text-xl md:text-2xl font-extrabold text-white">Fashion</h3>
//                     <p class="text-white/90 text-sm">Apparel, Shoes, Accessories</p>
//                 </div>
//             </div>
//         </a>`;
...
//     const imageMarkup = p.imageUrl 
//         ? `<div class="w-full h-32 md:h-40 overflow-hidden bg-white flex items-center justify-center border-b border-card-border/50"> 
//                <img src="${p.imageUrl}" alt="${p.name}" 
//                     class="w-full h-full object-cover transition duration-300" 
//                     onerror="this.onerror=null; this.outerHTML='<div class=&quot;w-full h-full bg-gray-200&quot;></div>';" /> 
//            </div>` 
//         : `<div class="w-full h-32 md:h-40 bg-gray-200 border-b border-card-border/50"></div>`; 


//     // 💡 FIXED LAYOUT: Enforced h-[360px] md:h-[390px] for stability and proper flex distribution 
//     return ` 
//     <div id="product-card-${p.id}" class="bg-white rounded-xl shadow-lg overflow-hidden product-card-hover border border-card-border reveal flex flex-col h-[360px] md:h-[390px] min-w-[15rem] flex-shrink-0 w-full"> 
...
//         <div class="p-3 md:p-4 flex flex-col flex-1 justify-between text-center"> 
...
//                 <h4 class="text-sm md:text-base font-semibold text-primary truncate-2-lines mt-1">${p.name}</h4> 
//                 <div class="flex justify-center items-center mt-2">${generateStarRating(p.rating || 0)}</div> 
                
//                 <p class="text-lg md:text-xl font-bold text-primary mt-3 pt-1"> 
//                     ${p.oldPrice ? `<span class="text-gray-500 line-through text-xs mr-2">${formatCurrency(p.oldPrice)}</span>` : ''} 
//                     <span id="${priceId}">${formatCurrency(initialUnit)}</span> 
//                     ${p.unit ? `<span class="text-xs md:text-sm text-gray-500 font-normal">/ ${p.unit}</span>` : ''} 
//                 </p> 
//                 ${savings > 0 ? `<p class="text-secondary-accent text-xs md:text-sm font-semibold mt-1">Save ${formatCurrency(savings)}</p>` : ''} 
...
//                 <div class="flex items-stretch border border-card-border rounded-full overflow-hidden h-full w-28 md:w-36"> 
//                     <button onclick="decBulkQty(${p.id}); return false;" class="px-2 md:px-3 h-full text-primary hover:text-secondary-accent flex items-center justify-center text-sm">-</button> 
//                     <input id="qty-selector-${p.id}" type="number" min="1" value="1" class="w-8 md:w-10 text-center outline-none h-full border-x border-card-border text-primary text-sm" oninput="onBulkQtyChange(${p.id}, this.value)" /> 
//                     <button onclick="incBulkQty(${p.id}); return false;" class="px-2 md:px-3 h-full text-primary hover:text-secondary-accent flex items-center justify-center text-sm">+</button> 
//                 </div> 
//                 <button onclick="addBulkToCart(${p.id}); return false;" class="flex-1 text-xs md:text-sm text-white bg-accent h-full px-3 md:px-4 rounded-full hover:bg-primary transition duration-150 flex items-center justify-center space-x-1"> 
...
//     header.innerHTML = `
//         <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0">
//             <h2 class="text-2xl md:text-3xl font-extrabold text-primary">Limited-Time Flash Deals</h2>
...
//     header.innerHTML = `
//         <h2 class="text-2xl md:text-3xl font-bold text-primary">${title} (${products.length} items)</h2>
//         <p class="text-sm text-gray-600 mt-2">${crumb}</p>
//     `;

//     banner.innerHTML = `
//         <div class="p-4 md:p-8 bg-white rounded-xl shadow-lg border-2 border-accent">
//             <p class="text-lg md:text-2xl font-bold text-primary">Browsing our <span class="text-accent">${title}</span></p>
//             <p class="text-gray-600 mt-2 text-sm">Curated highlights based on ratings and popularity.</p>
//         </div>
//     `;

//     // Subtle loading state
//     const skeletonCard = () => (
//         `<div class="bg-white rounded-xl shadow-lg border border-card-border p-3 md:p-4 animate-pulse w-full">
//             <div class="w-full aspect-[4/3] bg-gray-200 rounded-md"></div>
//             <div class="mt-3 h-3 md:h-4 bg-gray-200 rounded w-3/4"></div>
//             <div class="mt-2 h-3 bg-gray-100 rounded w-1/2"></div>
//             <div class="mt-4 h-8 md:h-10 bg-gray-100 rounded w-full"></div>
//         </div>`
//     );
...
//             return `
//                 <div class="bg-white rounded-xl shadow-lg border border-card-border p-3 md:p-4 hover:shadow-xl transition-all duration-300 reveal w-full">
//                     <div class="relative">
...
//                                     <i data-lucide="package" class="w-8 h-8 md:w-12 md:h-12 text-gray-400"></i>
//                                 </div>`
//                             }
//                         </div>
...
//                             <i data-lucide="heart" class="w-3 h-3 md:w-4 md:h-4 ${isInWishlist ? 'fill-red-500 text-red-500' : 'text-gray-600'}"></i>
//                         </button>
//                     </div>
//                     <div class="mt-3">
//                         <h3 class="font-semibold text-gray-900 text-sm leading-tight line-clamp-2">${p.name}</h3>
...
//                             <div>
//                                 <span class="text-base md:text-lg font-bold text-primary">$${p.price.toFixed(2)}</span>
...
//                                     <button onclick="decFilteredQty(${p.id})" class="p-2 hover:bg-gray-100 transition-colors flex-shrink-0">
//                                         <i data-lucide="minus" class="w-3 h-3 md:w-4 md:h-4"></i>
//                                     </button>
//                                     <span class="px-3 py-2 min-w-[3rem] text-center font-medium text-sm">${currentQty}</span>
//                                     <button onclick="incFilteredQty(${p.id})" class="p-2 hover:bg-gray-100 transition-colors flex-shrink-0">
//                                         <i data-lucide="plus" class="w-3 h-3 md:w-4 md:h-4"></i>
//                                     </button>
//                                 </div>` :
//                                 `<button onclick="addFilteredToCart(${p.id})" class="flex-1 bg-primary text-white py-2 px-4 rounded-lg hover:bg-primary-dark transition-colors font-medium text-sm w-full">
//                                     Add to Cart
//                                 </button>`
//                             }
//                         </div>
...
//         ? `<img src="${product.imageUrl}" alt="${product.name}" class="w-full h-48 md:h-72 object-cover rounded-xl" onerror="this.onerror=null; this.outerHTML='<div class=&quot;w-full h-48 md:h-72 rounded-xl&quot; style=&quot;background-color:${colorHex}&quot;></div>'" />`
//         : `<div class="w-full h-48 md:h-72 rounded-xl" style="background-color:${colorHex}"></div>`;

//     const tiersHtml = ensureBulkPricing(product).map(t => {
//         return `<div class="flex justify-between text-xs md:text-sm"><span>Buy ${t.minQuantity}+</span><span>${t.discountPercent}% off</span></div>`;
//     }).join('');
...
//             <div>${imageMarkup}</div>
//             <div class="space-y-4">
//                 <h2 class="text-2xl md:text-3xl font-bold text-primary">${product.name}</h2>
//                 <div class="mt-2 flex items-center">${generateStarRating(product.rating || 0)}</div>
//                 <p class="mt-2 text-gray-600 text-sm">${product.brand || ''} ${product.unit ? '• ' + product.unit : ''}</p>

//                 <div class="mt-6 p-4 md:p-4 rounded-xl border border-card-border bg-white">
//                     <div class="flex items-center space-x-2 md:space-x-3">
//                         <button id="pdp-qty-dec" class="w-8 h-8 rounded-full bg-gray-200 text-primary font-bold flex items-center justify-center">-</button>
//                         <input id="pdp-quantity" type="number" min="1" max="500" value="1" class="w-16 md:w-20 p-2 border rounded-md text-center text-sm" />
//                         <button id="pdp-qty-inc" class="w-8 h-8 rounded-full bg-gray-200 text-primary font-bold flex items-center justify-center">+</button>
//                     </div>
//                     <div class="mt-4">
//                         <p class="text-lg md:text-xl font-semibold">Per-unit: <span id="pdp-unit-price">${formatCurrency(product.price || 0)}</span></p>
//                         <p class="text-lg md:text-xl font-semibold">Total: <span id="pdp-total-price">${formatCurrency(product.price || 0)}</span></p>
//                         <p class="text-xs md:text-sm text-accent font-medium mt-1" id="pdp-bulk-callout">Bulk price applied where eligible</p>
//                     </div>
//                     <div class="mt-4 space-y-1">
//                         <h4 class="font-semibold text-primary text-sm md:text-base">Tiered Pricing</h4>
//                         ${tiersHtml}
//                     </div>
//                     <button id="pdp-add-to-cart" class="mt-6 w-full text-white bg-accent py-3 px-4 rounded-full hover:bg-primary transition duration-150 flex items-center justify-center space-x-2 text-sm">
